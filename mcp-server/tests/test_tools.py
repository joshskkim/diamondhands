"""Tool tests: verify each tool hits the expected endpoint/params and that HTTP
...
@respx.mock
def test_get_today_games_hits_endpoint():
    route = respx.get(f"{BASE}/api/games/today").mock(
        return_value=httpx.Response(200, json=[{"gameId": 1}])
    )
    assert server.get_today_games() == [{"gameId": 1}]
    assert route.called


@respx.mock
def test_get_best_plays_caps_limit_and_passes_date():
    route = respx.get(f"{BASE}/api/odds/best").mock(
        return_value=httpx.Response(200, json=[])
    )
    server.get_best_plays(date="2026-06-18")
...
@respx.mock
def test_omitted_date_is_not_sent():
    route = respx.get(f"{BASE}/api/props/board").mock(
        return_value=httpx.Response(200, json={})
    )
    server.get_prop_board()
    assert "date" not in route.calls.last.request.url.params


@respx.mock
def test_search_player_caps_limit():
    route = respx.get(f"{BASE}/api/players/search").mock(
        return_value=httpx.Response(200, json=[])
    )
    server.search_player(name="judge")
...
@respx.mock
def test_get_player_combines_detail_and_recent():
    ...
    result = server.get_player(player_id=592450)
...
@respx.mock
def test_path_params_interpolated():
    route = respx.get(f"{BASE}/api/games/42/projections").mock(
        return_value=httpx.Response(200, json={"ok": True})
    )
    assert server.get_game_projections(game_id=42) == {"ok": True}
    assert route.called


@respx.mock
def test_http_error_becomes_error_dict():
    respx.get(f"{BASE}/api/games/today").mock(return_value=httpx.Response(503))
    result = server.get_today_games()
    assert "error" in result
    assert "503" in result["error"]


@respx.mock
def test_network_error_becomes_error_dict():
