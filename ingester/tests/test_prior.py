...
class TestMarcelPrior(unittest.TestCase):
    ...
    def test_full_three_season_prior(self):
        ...
        # xwOBA: (5*600*.360 + 4*550*.350 + 3*500*.340 + 1200*.318) / (6700 + 1200)
        #      = (1080 + 770 + 510 + 381.6) / 7900 = 2741.6 / 7900
        self.assertAlmostEqual(prior.xwoba, round(2741.6 / 7900, 4), places=4)

        # K rate: each season is exactly .20 (k/pa), so weighted = .20.
        #      = (6700*.20 + 1200*LEAGUE_K) / 7900
        exp_k = (6700 * 0.20 + MARCEL_REGRESSION_PA * LEAGUE_K_PER_PA) / 7900
        self.assertAlmostEqual(prior.k_rate, round(exp_k, 4), places=4)

        # ISO per season = (tb-hits)/ab: .222222, .200000, .177778
        #      num = 3000*.222222 + 2200*.2 + 1500*.177778 = 666.667+440+266.667
        iso_num = 3000 * ((280 - 160) / 540) + 2200 * 0.20 + 1500 * ((200 - 120) / 450)
        exp_iso = (iso_num + MARCEL_REGRESSION_PA * LEAGUE_ISO) / 7900
    ...
    def test_single_prior_season_regresses_hard_to_league(self):
        ...
        assert prior is not None
        # weighted PA = 5*200 = 1000; regression adds 1200 of league.
        self.assertEqual(prior.proj_pa, 1000)
        exp_xwoba = (1000 * 0.420 + MARCEL_REGRESSION_PA * LEAGUE_XWOBA) / (1000 + MARCEL_REGRESSION_PA)
