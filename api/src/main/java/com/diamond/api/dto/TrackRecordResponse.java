 ...
 * quote was found, {@code clvRate} is the share with strictly positive CLV and {@code avgClv} the
 * mean CLV (de-vigged probability points we beat the close by). Positive CLV is the best
 * small-sample predictor of long-run edge — but it needs a few hundred picks to be trustworthy,
 * so {@code clvN} is reported alongside as the sample size and both are null until any CLV is
 * captured. {@code clvZeroN} counts exact ties with the close (they sit in clvRate's denominator
 * but not its numerator), and {@code clvN} vs {@code overall.n} exposes the coverage gap — picks
 * whose closing quote was never found (e.g. the line moved off our number) carry no CLV at all.
 */
public record TrackRecordResponse(
    ...
    List<RecordSummaryDto> byTier,     // Strong / Standard / Lotto
    List<RecordSummaryDto> byBook,     // per-sportsbook, largest slice first
    ...
    Double clvRate,                    // share of clvN with strictly positive CLV, or null when clvN == 0
    Double avgClv,                     // mean CLV over clvN, or null when clvN == 0
    Integer clvZeroN                   // exact ties with the close within clvN, or null when clvN == 0
) {}
